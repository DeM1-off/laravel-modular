...
| Check module boundaries / dependencies | `php artisan module:check --boundaries` (flags references into another module's internals and undeclared `requires`) |
...
| Generate a class in a module | `php artisan module:make-{controller\|model\|action\|request\|event\|listener\|job\|command\|factory\|seeder\|test} {Name} {Class} [--force]` |
| Generate a migration | `php artisan module:make-migration {Name} {name} [--table=]` |

### Rules

- **Conventions do the loading** — config, migrations, views, routes, `lang/`
  translations and artisan commands in `Console/` directories load automatically.
  Never add manual `loadMigrationsFrom`/`mergeConfigFrom`/`commands()` calls, and
  don't register a class generated by `module:make-command` anywhere — discovery
  picks it up.
- **One module depends on another?** Declare it in the dependent module's
  `module.json` as `"requires": ["Other"]` — load order then respects it and
  `module:check` verifies it. Cross-module references must go through the target
  module's `Contracts`/`Data`/`Events`/`Enums` namespaces, never its internals;
  run `module:check --boundaries` after wiring modules together.
