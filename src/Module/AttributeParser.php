 ...
 *
 * @phpstan-type Settings array{name: string|null, config: bool, migrations: bool, views: bool, routes: bool, commands: list<class-string>, binds: list<array{abstract: class-string, concrete: class-string, singleton: bool}>, listens: list<array{event: class-string, listener: class-string}>}
 */
final class AttributeParser
{
    ...
    public static function parse(string $providerClass): array
    {
        ...
        foreach ($reflection->getAttributes(Bind::class) as $attribute) {
            ...
            $binds[] = ['abstract' => $bind->abstract, 'concrete' => $bind->concrete, 'singleton' => $bind->singleton];
        }
