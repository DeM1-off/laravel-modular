...
use Dem1Off\LaravelModular\Console\ModuleSyncCommand;
...
final class LaravelModularServiceProvider extends ServiceProvider
{
    ...
    /**
     * Register each enabled module's PSR-4 namespace on the Composer loader, so a
     * module works by just existing — no Composer package, no root PSR-4. A
     * disabled module is skipped, so its classes don't autoload either.
     */
    private function registerModuleAutoloading(): void
    {
        ...
        foreach ($this->app->make(ModuleManager::class)->enabled() as $module) {
